  ...
  private void open() {
    ...
    try {
      for (String cameraId : manager.getCameraIdList()) {
        ...
        Timber.d(
            "Camera horizontal FOV %f deg",
            2 * Math.toDegrees(Math.atan(.5 * widthDim / focalLengths[0])));
        Timber.d(
            "Camera vertical FOV %f deg",
            2 * Math.toDegrees(Math.atan(.5 * heightDim / focalLengths[0])));
          ...
          new CameraCaptureSession.StateCallback() {
            ...
            public void onConfigured(@NonNull CameraCaptureSession captureSession) {
              try {
                ...
                previewRequestBuilder.set(
                    CaptureRequest.SENSOR_EXPOSURE_TIME, 1 * 1_000_000L); // ns
